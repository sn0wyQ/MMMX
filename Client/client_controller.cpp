...
void ClientController::StartGameEvent(const Event& event) {
  ...
  view_->AddGameStartedNotification();
...
void ClientController::KeyPressEvent(QKeyEvent* key_event) {
  ...
  if (native_key == Controls::kKeyC && model_->IsGameStarted()) {
...
void ClientController::LocalPlayerClearLevelEvent(const Event& event) {
  if (!model_->IsLocalPlayerSet()) {
    return;
  }
  model_->GetLocalPlayer()->ClearLevel();
}
